...
void Gauge_Field_Alloc_Mu_Fixed(double **gauge_field, int T, int L) {
	fprintf(stderr, "void Mu_Fixed_Gauge_Field_Alloc(...   -->   Trying to allocate %d M ...",
			T * L * L * L * 8 * sizeof(double) / 1000000);

	if ((*gauge_field = (double *) malloc(T * L * L * L * 8 * sizeof(double))) == NULL) {
...
void Gauge_Field_Alloc_Timeslice(double **gauge_field, int L) {
	fprintf(stderr, "void Timeslice_Gauge_Field_Alloc(...   -->   Trying to allocate %d M ...",
			L * L * L * 4 * 8 * sizeof(double) / 1000000);

	if ((*gauge_field = (double *) malloc(L * L * L * 4 * 8 * sizeof(double))) == NULL) {
		fprintf(stderr, "\nError: void Gauge_Field_Alloc(...\n");
		exit(EXIT_FAILURE);
	}
...
void Gauge_Field_Copy_Timeslice(double *timeslice_gauge_field_dst, double *gauge_field_src, int T, int L, int timeslice) {
	memcpy(timeslice_gauge_field_dst, gauge_field_src + ggi(get_index(timeslice, 0, 0, 0, T, L), 0),
