...
void InitializeRand(int seed) {
	if (seed < 1) {
		fprintf(stderr, "Error: void InitializeRand(...\n");
		fprintf(stderr, "seed <= 0\n");
		exit(EXIT_FAILURE);
	}

	// first parameter = 2 --> high quality
	rlxd_init(2, seed);
}
...
int IRand(int min, int max) {
	...
	// generate a random number in [0.0,1.0) and store it in d1
	ranlxd(&d1, 1);

	int i1 = min + (int) ((max - min + 1) * d1);
...
void testlx() {
	...
	/*
	 ...
	 exit(0);
	 */
	fprintf(stderr, "void test_ranlx(): ranlxs and ranlxd work correctly on this machine.\n");
